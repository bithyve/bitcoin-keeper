...
function Element(props) {
  ...
  const btnBackTheme = colorMode === 'dark' ? containerBackgroundColorDark : containerBackgroundColorLight
  const textColor = colorMode === 'dark' ? '#24312E' : '#FAFCFC'
  const textActiveColor = colorMode === 'light' ? '#2A6255' : '#E3BE96'
  return (
    <Box borderRadius={15} style={props.isActive ? styles.containerBtnActive : styles.containerBtn} backgroundColor={props.isActive ? btnActiveBack : btnBackTheme}>
