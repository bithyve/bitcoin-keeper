    ...
    const html = `
        <html>
          ...
          <body>
            ...
            <p>${fingerPrints.map((keys, index) => `<p>Key ${index + 1} Fingerprint: ${keys}</p>`).join("")}</p>
            <p>These fingerprints act as unique identifiers for the respective keys without revealing sensitive details. Following the BIP32 (Bitcoin Improvement Proposal 32) standard, each fingerprint helps identify the associated extended public key (xPub). The xPub serves as a distinct identifier that can be utilized by a digital asset expert or software, adhering to standard BIP32 derivation paths, to locate and validate the keys during the transfer process.</p>
            ...
            <p>These fingerprints and the accompanying explanation of their usage can be recorded in my will if it is of any help. Further details regarding the designated beneficiary, executor, and supplementary instructions will be provided separately during our estate planning discussions. I have complete faith in your expertise to handle this confidential information securely. Please do not hesitate to contact me if you require additional documentation or information. Your support and meticulous attention to detail in facilitating the transfer of the legal title to my bitcoin holdings are greatly appreciated.</p>
