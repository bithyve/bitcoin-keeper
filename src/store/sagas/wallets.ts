...
import { VaultShell } from 'src/core/wallets/interfaces/vault';
...
export function* addNewVaultWorker({ payload: newVaultInfo }: { payload: newVaultInfo }) {
  const { vaultType, vaultDetails, signerDetails } = newVaultInfo;
  const vault: Vault = {};
