...
export async function connect() {
  ...
  try {
      ...
      electrumClient = new ElectrumCli(global.net, global.tls, peer.ssl || peer.tcp, peer.host, peer.ssl ? 'tls' : 'tcp') ;// tcp or tls
...
async function getCurrentPeer() {
  ...
  return isTestnet ? predefinedTestnetPeers[currentPeerIndex] : predefinedPeers[currentPeerIndex];
}

async function getNextPeer() {
  const isTestnet = config.NETWORK_TYPE == NetworkType.TESTNET;
  currentPeerIndex++;
  if (currentPeerIndex + 1 > (isTestnet ? predefinedTestnetPeers.length : predefinedPeers.length)) {
      currentPeerIndex = 0;
  }
