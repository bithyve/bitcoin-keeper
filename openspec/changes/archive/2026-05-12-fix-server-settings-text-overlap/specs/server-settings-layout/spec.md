...
The Server Settings screen SHALL render the no-connection `WarningNote` inside the scrollable server list (as a list footer) so that it always appears below the last server item and never obscures any action button.
...
- **THEN** the `WarningNote` SHALL appear below the last `ServerItem` within the scroll area
...
- **THEN** no `WarningNote` SHALL be rendered inside the list footer
