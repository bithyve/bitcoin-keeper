...
class CloudBackupModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext),
    ...
    fun backupBsms(data: String, password: String, promise: Promise) {
        try {
            ...
            document.add(image)
            val font = Font(Font.FontFamily.HELVETICA, 24f, Font.BOLD)
            val header = Paragraph("Your Wallet Configurations", font)
            header.alignment = Element.ALIGN_CENTER
            header.spacingBefore = padding
            header.spacingAfter = padding
            val paragraph1 = Paragraph("Dear Recipient,")
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                preface.add(Paragraph("Wallet Configuration Text:"))
                preface.add(Paragraph(" "));
                preface.add(Paragraph(content))
                preface.add(Paragraph("File Details: File created on: $formattedDateTime"));
                preface.add(Paragraph(" "));
